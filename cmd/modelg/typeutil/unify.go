// Copied from https://go.dev/src/go/types/unify.go since not
// publicly exposed, and modified to compile
...
func (u *unifier) tracef(format string, args ...interface{}) {
	fmt.Println(strings.Repeat(".  ", u.depth) + fmt.Sprintf(format, args...))
}
...
func (u *unifier) nify(x, y types.Type, mode unifyMode) (result bool) {
	...
	if ny := asNamed(y); mode&exact == 0 && ny != nil && isTypeLit(x) {
		if traceInference {
	...
	case *types.Chan:
		...
		if y, ok := y.(*types.Chan); ok {
			return (mode&exact == 0 || x.Dir() == y.Dir()) && u.nify(x.Elem(), y.Elem(), emode)
		}
	...
	case *types.Named:
		...
		if y := asNamed(y); y != nil {
			...
			for i := range xargs.Len() {
				xarg := xargs.At(i)
				if !u.nify(xarg, yargs.At(i), mode) {
					return false
				}
			}
